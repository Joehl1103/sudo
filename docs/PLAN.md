...
Initial decisions, open to experiment: block headers end in a colon; any consistent space indentation is accepted and formatted to four spaces; tabs are errors; natural-language actions/predicates are opaque; one workflow per file. `@WORKFLOW:` declares the file's single global workflow object; the filename provides its human-readable name, and downstream references use `@WORKFLOW`. STOP must explicitly target RECORD or `@WORKFLOW`. No credentials, network calls, generated automation scripts, or external mutations from authored workflows.
