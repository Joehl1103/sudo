...
- Current: v0 structural linter, formatter, CLI, Neovim plugin, and optional VS Code extension. The header declares one global `@UPPERCASE` workflow symbol; DEFINE and FOR EACH declare scoped `$UPPERCASE` local values. Joseph uses Neovim; keep it the primary editor.
